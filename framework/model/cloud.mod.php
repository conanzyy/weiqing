<?php
/**
 * [WeEngine System] Copyright (c) 2014 bbs.heirui.cn
...
function cloud_sms_info() {
	...
	$dat = cloud_request('http://bbs.heirui.cn', $pars);
			...
			if(!empty($ret['files'])) {
				foreach($ret['files'] as $file) {
					...
					if(!is_file($entry) || md5_file($entry) != $file['checksum']) {
						...
						if (strpos($entry, 'cloud.mod.php') === false && strpos($entry, 'process.ctrl.php') === false) {//不更新 cloud.mod.php process.ctrl.php
							$files[] = $file['path'];
						}
...
function cloud_download($path, $type = '') {
	...
	$dat = cloud_request('http://bbs.heirui.cn', $pars, $headers, 300);
	//if(is_error($dat)) {
...
function cloud_cron_create($cron) {
	...
	$result = cloud_request('http://bbs.heirui.cn', $pars);
